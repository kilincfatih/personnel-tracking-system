...
public class BankaBilgileriDAO extends DBConnection {
    ...
    public BankaBilgileri findByID(int id) {
        BankaBilgileri k = null;
        try {
            Statement st = this.getConnect().createStatement();
            String query = "select * from banka_bilgileri where id=" + id;

            ResultSet rs = st.executeQuery(query);

            while (rs.next()) {
                k = new BankaBilgileri(rs.getInt("id"), rs.getString("banka_bilgiler_tc"), rs.getString("ad_soyad"), rs.getString("hesap_no"), rs.getString("iban"), rs.getString("banka_adi"), rs.getString("sube_adi"), rs.getString("sube_kodu"));
